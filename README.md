...
Schemas: Contém os comandos CREATE TABLE para a criação de todas as tabelas e suas respectivas chaves primárias (PK) e estrangeiras (FK).

Inserts: Contém os comandos INSERT INTO para popular as tabelas principais com dados de exemplo.

Selects: Contém exemplos de consultas avançadas (SELECT) utilizando JOIN, WHERE, ORDER BY, GROUP BY, HAVING e LIMIT.

Update/Delete: Contém exemplos de comandos de manipulação de dados (UPDATE e DELETE) com condições.
...
Crie o Banco de Dados: Crie um novo banco de dados vazio no seu SGBD (Sistema Gerenciador de Banco de Dados).

Crie o Esquema: Execute o script schemas para criar todas as tabelas.

Popule os Dados: Execute o script de inserts para inserir os dados de exemplo nas tabelas.

Execute as Consultas: Execute o script de selects para testar as consultas e visualizar os resultados.

Manipule os Dados: Execute o script de update/delete para aplicar as atualizações e exclusões de dados.

Scripts SQL Solicitados
